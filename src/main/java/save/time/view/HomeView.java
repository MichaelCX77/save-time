...
public class HomeView {
  ...
  public static void carregaHome() {
    ...
    lblBemVindo = new JLabel("Bem vindo(a), " + UserSession.getUser().getNome().substring(0, UserSession.getUser().getNome().lastIndexOf(" ")) + "!");
  ...
  public static void verificaModificacoes() {
    if (recarrega) {
      recarrega = false;
    } else if (LeiturasPanel.isControle() || ExerciciosPanel.isControle() || IdiomasPanel.isControle() || EspiritualPanel.isControle() || DietasPanel.isControle() || CursosPanel.isControle()) {
      Object[] options = { "Prosseguir", "Cancelar" };
      int option = JOptionPane.showOptionDialog(getPanelHome(), "Existem dados n\u00E3o salvos, deseja prosseguir e perd\u00EA-los?", "Mensagem", -1, 1, null, options, options[0]);
      if (option == 0) {
