...
public class LeiturasPanel {
  ...
  public static void carregarLeituras() {
    ...
    table.setModel(new DefaultTableModel(LeiturasPanelController.carregaTable(), 
          (Object[])new String[] { "Data", "Tempo Dedicado", "Livro", "Total de P\u00E1ginas", "P\u00E1gina Atual", "Conclus\u00E3o" }) {
    ...
    btnFinalizarTudo.addActionListener(new ActionListener() {
          public void actionPerformed(ActionEvent e) {
            if (!LeiturasPanel.txtPaginaAtualFinalizar.getText().matches("[0-9]*") || LeiturasPanel.txtPaginaAtualFinalizar.getText().equals("")) {
              LeiturasPanel.lblMensagemFinalizar.setText("*Verifique p\u00E1gina atual");
              LeiturasPanel.lblMensagemFinalizar.setVisible(true);
            } else if (Integer.parseInt(LeiturasPanel.txtTotalPaginasFinalizar.getText()) < Integer.parseInt(LeiturasPanel.txtPaginaAtualFinalizar.getText())) {
              LeiturasPanel.lblMensagemFinalizar.setText("*Verifique p\u00E1gina atual");
              LeiturasPanel.lblMensagemFinalizar.setVisible(true);
            } else {
              Leituras livro = LeiturasPanelController.verificaPaginaMaisRecente(UserSession.getUser().getId(), LeiturasPanel.txtNomeLivroFinalizar.getText());
              if (livro.getPgAtual() >= Integer.parseInt(LeiturasPanel.txtPaginaAtualFinalizar.getText())) {
                LeiturasPanel.lblMensagemFinalizar.setText("*Verifique p\u00E1gina atual");
                LeiturasPanel.lblMensagemFinalizar.setVisible(true);
              } else {
                LeiturasPanel.objLeituras.setPgAtual(Integer.parseInt(LeiturasPanel.txtPaginaAtualFinalizar.getText()));
    ...
    comboMeusLivros.addItemListener(new ItemListener() {
          public void itemStateChanged(ItemEvent e) {
            if (e.getItem().toString().equals("Selecione...")) {
              ...
              Leituras livro = LeiturasPanelController.verificaPaginaMaisRecente(UserSession.getUser().getId(), e.getItem().toString());
  ...
  public static void actionProximo(ActionEvent e) {
    if (txtNomeLivro.getText().equals("") && comboMeusLivros.getSelectedItem().equals("Selecione...")) {
      ...
      String livro = LeiturasPanelController.verificaLivroString(UserSession.getUser().getId(), txtNomeLivro.getText());
      if (!livro.equals("")) {
        lblMensagem.setText("*Voc\u00EA j\u00E1 cadastrou esse livro!");
        lblMensagem.setVisible(true);
      } else {
        boolean livroFinalizado = false;
        if (!txtPaginaAtualForm.getText().equals("") && !txtTotalPaginasForm.getText().equals("") && 
          Integer.parseInt(txtPaginaAtualForm.getText()) == Integer.parseInt(txtTotalPaginasForm.getText()))
